...
data = dict(
    ...
    train=dict(
        type='CocoDataset',
        classes=('crater', ),
        ann_file='data/DATASETS/Craters/annotations/instances_train2017.json',
        img_prefix='data/DATASETS/Craters/train2017/',
        pipeline=[
            dict(type='LoadImageFromFile', color_type='color'),
            dict(type='LoadAnnotations', with_bbox=True),
            dict(type='Resize', img_scale=[(768, 768)], keep_ratio=False),
    ...
    val=dict(
        type='CocoDataset',
        classes=('crater', ),
        ann_file='data/DATASETS/Craters/annotations/instances_val2017.json',
        img_prefix='data/DATASETS/Craters/val2017/',
        pipeline=[
            dict(type='LoadImageFromFile', color_type='color'),
            dict(type='Resize', img_scale=[(768, 768)], keep_ratio=False),
            dict(
...
lr_config = dict(
    ...
    warmup_ratio=0.001,
    min_lr=0)
runner = dict(type='EpochBasedRunner', max_epochs=50.0)
...
model = dict(
    ...
    test_cfg=dict(
        ...
        max_per_img=100))
work_dir = 'checkpoints/Craters/retina18/retina18_2022-08-02T01:17:25_768_50.0e_Craters_lr_0.001_CosineAnnealing'
auto_resume = True
gpu_ids = range(0, 1)
