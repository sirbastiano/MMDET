...
data = dict(
    ...
    train=dict(
        type='CocoDataset',
        classes=('wake', ),
        ann_file='data/DATASETS/B8/annotations/instances_train2017.json',
        img_prefix='data/DATASETS/B8/train2017/',
        pipeline=[
            dict(type='LoadImageFromFile', color_type='color'),
            dict(type='LoadAnnotations', with_bbox=True),
            dict(type='Resize', img_scale=[(768, 768)], keep_ratio=False),
    ...
    val=dict(
        type='CocoDataset',
        classes=('wake', ),
        ann_file='data/DATASETS/B8/annotations/instances_val2017.json',
        img_prefix='data/DATASETS/B8/val2017/',
        pipeline=[
            dict(type='LoadImageFromFile', color_type='color'),
            dict(type='Resize', img_scale=[(768, 768)], keep_ratio=False),
            dict(
...
model = dict(
    ...
    test_cfg=dict(
        ...
        max_per_img=100))
work_dir = 'checkpoints/B8/retina50/retina50_2023-02-10T21:01:20_768_50.0e_B8_lr_0.001_step'
auto_resume = True
gpu_ids = range(0, 1)
